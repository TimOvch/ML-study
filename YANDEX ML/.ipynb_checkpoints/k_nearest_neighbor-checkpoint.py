...
class KNearestNeighbor:
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            squared_diff = np.square(X[i] - self.X_train)
    ...
    def compute_distances_no_loops(self, X):
        ...
        X_train_norm = np.reshape(np.sum(self.X_train ** 2, axis=1), (1, -1))
        dot_prod = np.dot(X, self.X_train.T)
        dists = np.sqrt(X_norm + X_train_norm -2*dot_prod)
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            labels = self.y_train[closest_indices]
